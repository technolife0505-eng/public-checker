...
(function(){
  function closeAll(except){
    document.querySelectorAll(".ms-menu").forEach(function(menu){
      if(menu !== except){ menu.classList.remove("open"); menu.style.display="none"; }
    });
  }
  window.mmToggleMS = function(btn){
    ...
    closeAll(menu);
    if(isOpen){ menu.classList.remove("open"); menu.style.display="none"; }
    else{ menu.classList.add("open"); menu.style.display="block"; }
    return false;
  };
  function title(g){
  ...
  function updateLabel(g){
    ...
    if(!label) return;
    if(allBox && allBox.checked) label.textContent=title(g)+": Barchasi";
    else if(checked.length===0){ if(allBox) allBox.checked=true; label.textContent=title(g)+": Barchasi"; }
    else label.textContent=title(g)+": "+checked.length+" ta tanlandi";
  }
  ...
  document.addEventListener("change", function(e){
    ...
    if(all){
      ...
      if(all.checked) document.querySelectorAll('[data-group-item="'+g+'"]').forEach(function(x){x.checked=false;});
    }
    if(item){
