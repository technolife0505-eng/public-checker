...
def parse_posts(html, username, clean_link):
    ...
    for block in soup.select(".tgme_widget_message"):
        data=block.get("data-post","")
        mid=data.split("/")[-1] if "/" in data else data
        ...
        if text and mid:
            posts.append({"platform":"telegram","username":username,"source_link":clean_link,"source_title":title,"message_id":str(mid),"message_url":f"https://t.me/{username}/{mid}","text":text,"message_time":dt})
    return posts
...
def fetch_telegram_between(link, date_from=None, date_to=None, max_pages=80):
    ...
    if date_from: start=datetime.fromisoformat(str(date_from)).replace(tzinfo=timezone.utc)
    else: start=datetime.now(timezone.utc)-timedelta(days=2)
    if date_to: end=datetime.fromisoformat(str(date_to)).replace(hour=23,minute=59,second=59,microsecond=999999,tzinfo=timezone.utc)
    else: end=datetime.now(timezone.utc)
...
def scan_source_for_keywords(source, keywords, date_from=None, date_to=None):
    if (source.platform or "telegram").lower()!="telegram":
        return []
    posts=fetch_telegram_between(source.link,date_from,date_to)
    results=[]
    for p in posts:
        for kw in keywords:
