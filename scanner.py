...
from ai_rules import classify_full
...
def scan_source_for_keywords(source,keywords,date_from=None,date_to=None):
    ...
    for p in posts:
        for kw in keywords:
            ...
            if cnt>=1:
                ai_classification, sent, risk = classify_full(p['text'], kw.user_category)
                results.append({**p,'source_language':source.language,'matched_keyword':kw.keyword,'user_category':kw.user_category,'repetition_count':cnt,'ai_classification':ai_classification,'sentiment':sent,'risk_level':risk})
    return results
