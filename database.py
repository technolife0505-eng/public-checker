...
def update_source(i, platform, link, username, language='UZ', category=None):
    ...
    try:
        row=s.query(Source).filter(Source.id==i).first()
        if row:
            row.platform=platform; row.link=link; row.username=username; row.language=language; row.category=category
            s.commit()
    finally: s.close()
...
def update_keyword(i, keyword, user_category=None, priority='MEDIUM'):
    ...
    try:
        row=s.query(Keyword).filter(Keyword.id==i).first()
        if row:
            row.keyword=(keyword or '').lower().strip(); row.user_category=user_category; row.priority=priority
            s.commit()
    finally: s.close()
